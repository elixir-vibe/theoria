...
| Matcher.Type | Plans and validates descriptor-derived `Matcher.Type.Shape` values with parameters, indices, motive binders/results, discriminants, alternatives, derived case binders, index patterns, constructor applications, per-constructor case results, recursor arguments, and recursor application bodies, then folds simple non-indexed telescopes; not a general dependent matcher compiler. |

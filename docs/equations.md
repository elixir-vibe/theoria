...
- indexed Vec-like matcher descriptors can be represented and planned into `Matcher.Type.Shape` from `vec_ind`, including constructor fields, recursive fields, index binders, dependent motive binders/results, and per-alternative index patterns, but `Matcher.Type` intentionally rejects checked declaration emission until dependent matcher generation exists;
...
| Matcher.Type | Plans descriptor-derived `Matcher.Type.Shape` values with parameters, indices, motive binders/results, discriminants, alternatives, derived case binders, index patterns, result, recursor arguments, and body, then folds simple non-indexed telescopes; not a general dependent matcher compiler. |

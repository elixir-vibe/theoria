...
- indexed Vec-like matcher descriptors can be represented from `vec_ind`, including constructor fields, recursive fields, and index patterns, but `Matcher.Type` intentionally rejects them until dependent matcher generation exists;
...
| Matcher.Type | Plans descriptor-derived `Matcher.Type.Shape` values with parameters, motive, discriminants, alternatives, derived case binders, result, recursor arguments, and body, then folds simple telescopes; not a general dependent matcher compiler. |

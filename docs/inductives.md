...
Indexed families can declare named indices. The current checker validates result arity, parameter preservation, and scopedness of constructor argument/index terms. `Theoria.Inductive.Generate.indexed_eliminators/1` can synthesize opaque indexed induction declarations, but indexed iota reduction is not implemented yet; generated indexed eliminators have `reduction: nil`, install as `:recursor` declarations, and carry `Theoria.Env.Recursor` metadata with empty rules. Environment-backed admission also requires dependencies to be present. For example, this Vec-like spec must be checked in an environment that already contains `Nat`, `zero`, and `succ`.

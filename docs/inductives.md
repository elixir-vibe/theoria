...
The built-in `Bool`, `Nat`, and `List` libraries use this same path. Their constructor targets are decomposed with `Theoria.Inductive.Constructor.result/2`; non-dependent recursor types and generic reduction metadata are derived from that decomposition. Dependent induction types are still specialized for now. Generated eliminators carry generic `Theoria.Env.Reduction.Recursor` metadata so normalization can select a constructor branch and pass recursive calls without relying on recursor-specific reducer names.

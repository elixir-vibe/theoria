...
| Equation lemmas | Generated simplification theorems | 🟡 | `Compiled` + `DefinitionSpec` + compiler-owned validated schema-backed schematic generated lemmas for supported Bool/Nat/List definitions; derived matcher metadata; `Eqns` lookup/install/rules/database API; Lean oracle proof coverage; opt-in theorem installation; rewrite DB construction from env metadata | General matcher equation generation and theorem sets still needed for full `simp` | P2 |

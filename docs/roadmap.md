...
Status: initial finite-set, graph path, effect lattice, and typespec compatibility vocabularies started in `0.9.0-dev` with `Theoria.Spec.Finite`, `Theoria.Spec.Graph`, `Theoria.Spec.Effect`, `Theoria.Spec.Typespec`, the `Theoria.Spec.Claim` protocol, and `Theoria.Spec.Report`.

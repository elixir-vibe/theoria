  ...
  def run(args) do
    ...
    case Validation.check(corpus) do
      {:ok, result} ->
        ...
        Mix.shell().info(
          "✓ indexed matcher equation statements: #{result.indexed_matcher_statement_count} type(s)"
        )

        Mix.shell().info(
          "✓ indexed matcher equation lemmas: #{result.indexed_matcher_lemma_count} metadata entrie(s)"
        )
